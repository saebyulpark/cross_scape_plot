...
% This function compares all segments of the two songs to find the minimum distance matrix for each unit and plots it as a cross-scape plot. 
% Note that the cross-scape plot is the center-aligned graph of the minimum distance matrix.  
% dist_mat = get_distance_matrix(song_a, song_b, 1) obtains the minimum distance matrix between the pitch intervals of two songs.
% dist_mat = get_distance_matrix(song_a, song_b, 2) obtains the minimum distance matrix between the rhythm ratio of two songs.
